...
Ability.prototype.map_target = function(path) {
	...
	// replace the rest of '/' with -, to match the setup [removed]
  // target = target.replace(/\//g, '-');
  
  // remove all domains except first
  var indexOfSlash = target.indexOf('/');
  if(indexOfSlash > -1) {
    target = target.substr(0, indexOfSlash);
  }
  
  // remove any format specifiers from domain
  var indexOfDot = target.indexOf('.');
  if(indexOfDot > -1) {
    target = target.substr(0, indexOfDot);
  }
